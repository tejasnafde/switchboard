...
## Performance policy (RAM is P0)

- ONE code-server process per app, ever (validated: serves any folder). Never per-project.
- ONE `<webview>` total, reused across projects: switching projects navigates the same webview to the new `?folder=`. Cost: a workbench reload on project switch; layout persists via the `persist:ide` partition.
- Idle shutdown: hidden 15 minutes → kill the server, blank the webview to `about:blank`. Cold respawn is fast (~0.35s to /healthz, live-probed).
- Nothing spawns at app launch; first IDE-pane open pays the one-time download and the spawn.
- Event-driven everywhere: no polling loops in manager or bridge; `/healthz` polled only during boot, capped retries. Webview `backgroundThrottling` stays on.
- Bundle: binary downloaded on demand to userData, never in the dmg; sb-bridge is two small JS files; CodeMirror/LSP deps dropped from the renderer build.

Actual RSS/CPU numbers are deliberately NOT claimed here - measuring the whole app is a separate audit.
