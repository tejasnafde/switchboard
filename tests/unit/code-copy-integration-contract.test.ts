...
describe('code-copy integration contract', () => {
  it('keeps copy ownership in the shared renderer instead of post-processing DOM walks', () => {
    ...
    expect(markdown).toContain('getSnapshotBeforeUpdate(')
    expect(markdown).toContain('focusedCopyIndexBeforeReplacement(')
    expect(markdown).toContain('componentDidUpdate(')
