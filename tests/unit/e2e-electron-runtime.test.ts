...
describe('Electron E2E runtime isolation', () => {
  it('copies native modules for Electron rebuilding without mutating the Node test install', async () => {
    ...
    mkdirSync(join(repoRoot, 'node_modules', 'better-sqlite3'), { recursive: true })
    ...
    writeFileSync(join(repoRoot, 'node_modules', 'better-sqlite3', 'binding.node'), 'node-abi')
    ...
    expect(readFileSync(join(repoRoot, 'node_modules', 'better-sqlite3', 'binding.node'), 'utf8')).toBe('node-abi')
