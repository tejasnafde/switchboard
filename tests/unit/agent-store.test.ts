...
describe('agent-store', () => {
  ...
  it('resetRunningSessionsForMachine idles only running sessions on that machine', () => {
    ...
    addSession({ id: 'b', type: 'claude-code', status: 'idle', machineId: 'm1' })
    addSession({ id: 'c', type: 'claude-code', status: 'running', machineId: 'm2' })
    addSession({ id: 'd', type: 'claude-code', status: 'running' }) // local, no machineId

    resetRunningSessionsForMachine('m1')

    const byId = Object.fromEntries(useAgentStore.getState().sessions.map((s) => [s.id, s.status]))
    expect(byId).toEqual({ a: 'idle', b: 'idle', c: 'running', d: 'running' })
  })
