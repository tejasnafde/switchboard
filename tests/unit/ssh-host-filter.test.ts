...
const hosts: SshHost[] = [
  { alias: 'acme-ssg-bot-stg-in', hostName: 'acme-ssg-bot-stg-in', user: 'deploy_acme_io', port: 22 },
  { alias: 'prod-db', hostName: '10.0.0.4', user: 'ubuntu', port: 22 },
  { alias: 'acme-stg', hostName: 'compute.7364', port: 22 },
]
...
describe('filterSshHosts', () => {
  ...
  it('matches on alias', () => {
    expect(filterSshHosts(hosts, 'ssg').map((h) => h.alias)).toEqual(['acme-ssg-bot-stg-in'])
  })
