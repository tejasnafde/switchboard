/**
 * Tests for `sortLaunchConfigsByRecency` - picker dropdown sort key.
 *
...
describe('sortLaunchConfigsByRecency', () => {
  it('returns alphabetical with `default` pinned to top when no recorded use', () => {
    const out = sortLaunchConfigsByRecency(['zebra', 'apple', 'default', 'mango'], '/proj')
  ...
  it('scopes recency per-project', () => {
    recordLaunchConfigUsage('/proj-a', 'staging')
    const out = sortLaunchConfigsByRecency(['default', 'staging'], '/proj-b')
  ...
  it('does not mutate the input array', () => {
    ...
    const out = sortLaunchConfigsByRecency(input, '/proj')
