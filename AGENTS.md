...
Electron workspace that multiplexes terminals, agent chats (Codex + Codex + OpenCode), a kanban board, and an embedded VS Code IDE into one surface per project.
...
- **Agents** (3, all behind the `ProviderAdapter` interface):
  - Codex via `@anthropic-ai/Codex-agent-sdk` (streaming-input mode, AsyncIterable prompt queue, `canUseTool` callback)
...
- `ELECTRON_RUN_AS_NODE=1` is set by Codex's shell - `dev` script unsets it explicitly
- `electron` MUST be in `devDependencies`, not `dependencies`
- After `npm install`, run `npm run rebuild` for `node-pty` + `better-sqlite3`
- Codex encodes project paths by replacing BOTH `/` and `_` with `-` in `~/.Codex/projects/`
...
- `ProviderKind = 'Codex' | 'codex' | 'opencode'`
...
- **`policy.ts` is the shared policy module** (2026-04/05 - was previously inlined in Codex-adapter). All three adapters import from it:
...
- `Codex-session-migrate.ts` - copies Codex SDK session JSONL across `oauth_dir` when rotating provider instances mid-flight, so resume survives a credential switch
...
4. Codex adapter strips the `data:image/png;base64,` prefix and builds `{type:'image', source:{type:'base64', media_type, data}}` content blocks alongside text
...
- IPC (`ProviderInstanceChannels`): `LIST` (secrets stripped), `UPSERT` (plaintext env in, encrypted at rest), `DELETE` (refuses last-of-kind), `TEST` (probes creds via `Codex auth status` / `codex login status` / `opencode models`), `CREATE_OAUTH_DIR`.
...
- Codex SDK integration end-to-end: streaming text, tool calls, context window metrics, interrupt
...
- **Slash command menu in chat input** with agent-skill exposure (2026-04-26): Codex SDK `init.commands` + Codex `skills/list` + OpenCode `available_commands_update` surfaced alongside Switchboard's 9 built-ins. Source-grouped sections in the menu; agent-source selections insert `/<name> ` for the user to fill in args.
...
- **Per-turn duration badge** (2026-04-29): adapters stamp `turnStartedAt` on `sendTurn` and emit `durationMs` on `turn.completed`. MessageBubble renders "Worked for X.Xs" under the assistant message via `fmtDuration` from `src/shared/format.ts`. Wired across all 3 active adapters (Codex, codex, opencode-acp).
...
- **Agent-aware UI labels**: `agentLabel()` / `agentShortLabel()` helpers in `shared/types.ts` so StatusBar / MessageBubble / etc. all reflect Codex / Codex / OpenCode correctly
...
- **Rate-limit event handling** (2026-06-10): Codex SDK `rate_limit_event` surfaced as a chat status message with window type + reset time; subprocess leak on `stopSession` fixed (6 new tests in `Codex-adapter-stop-session.test.ts`)
...
- **Codex adapter** captures `system/init.{slash_commands|commands}` and prefers live `query.supportedCommands()`. Cached on the active session.
...
Pure parsers exported and unit-tested: `parseClaudeSlashCommands` (Codex-adapter), `parseCodexSkills` (codex-adapter), `mergeWithAgentSkills` + `skillsToSlashCommands` (slashCommands.ts).
...
- `Codex-adapter-plan-mode.test.ts` / `provider-policy.test.ts` - plan mode permission policy
- `provider-adapter-tool-filter.test.ts` - `CUSTOM_UI_TOOLS` set membership
- `jsonl-parser.test.ts` / `jsonl-truncate.test.ts` - Codex + Codex schemas, historical images, fork truncation
...
│   │   ├── jsonl-parser.ts            # Source-aware (Codex | codex) + image extraction
...
│   ├── projects/
│   │   ├── session-scanner.ts         # exact-match Codex + Codex scanners (encodeClaudeProjectPath)
...
│   │   ├── env-overlay.ts             # instance env merge · Codex-session-migrate.ts # oauth_dir rotation
│   │   ├── types.ts                   # ProviderAdapter + re-exports from shared/provider-events
│   │   └── adapters/
│   │       ├── Codex-adapter.ts      # SDK integration, canUseTool, image blocks
