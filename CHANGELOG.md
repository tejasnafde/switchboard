...
### Fixed
- **Settings' Accounts & models page holds still while usage loads.** Cards were re-sorted by room left as each reading landed, grew when their bars appeared, and blanked on refresh. Usage reads now start when Settings opens, on any page, and the readings stay in the renderer so a reopened page shows them at once. Cards reserve their bar height with a skeleton, keep their order while the page is open (sorted by the readings at hand when first shown), and keep the previous values dimmed during a refresh; the summary tiles show a placeholder rather than "None" until every first reading is in. `e2e/accounts-no-shift.e2e.mjs` checks every card's box frame by frame. See `docs/feature-parity/accounts-no-layout-shift.json`.
