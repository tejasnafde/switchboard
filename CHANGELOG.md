...
### Added
- **A safe worktree manager in Settings > Archive & data.** "Review" lists every worktree across projects with its branch, project, linked chat, size on disk (measured lazily off the UI thread and cached) and git state, under All / Safe to remove / Has changes / In use tabs. Clean states are plain muted text; only uncommitted files or unpushed commits get amber and an icon, and an in-use worktree has a lock and cannot be ticked. Only worktrees that lose nothing join a batch; one with changes is removed on its own after a confirm naming what goes, and the backend re-checks that against fresh git state and uses `git worktree remove`, never deleting a branch that holds unpushed commits. A project (Settings > Projects) or a single worktree can be protected so it is never offered for cleanup or counted as stale; the flag lives on the backend, so a remote one honours it. The kanban board's worktree dialog is the same manager scoped to its project, and its old stale-removal channel now goes through the same guard. See `docs/feature-parity/settings-worktree-manager.json`.
- **Settings is a full-window page with search.** The modal's seven tabs become nine pages in a left navigation (General, Appearance, Chat & agents, Accounts & models, Projects, Keyboard, Devices & machines, Archive & data, About), with every setting moved as it was. One search covers every row on every page and opens the result's page with the row highlighted; a setting changed from its default shows a muted "Changed" beside its name (the tooltip names the default) and a Reset, and the navigation counts changed settings per page. The Keyboard page lists every command in the shortcut registry with this computer's keys. The privacy text links to the privacy policy, opened in the browser. Escape (after clearing a search) or Back returns to where you were. See `docs/feature-parity/settings-page-shell.json`.
- **One keyboard shortcut registry, and Ctrl stays with the terminal on macOS.** Shortcuts were defined in the global resolver, the app menu, the Cmd+W intercept, eight components and a hand-typed Settings list that had drifted (it missed ⌘⇧O, ⌘⇧K, ⌘⇧E, ⌘⇧J, ⌘F and ⌥Enter). They now live in `src/shared/shortcuts.ts` with an id, label, group, scope and a `Mod` binding, and every one of those places reads from it; Settings and the palette show ⌘⇧⌥ on macOS and Ctrl/Shift/Alt elsewhere. On macOS `Mod` now matches only ⌘: before, the window capture listener also took Ctrl, so in a focused terminal Ctrl+K opened the quick prompt, Ctrl+J hid the terminal and Ctrl+B toggled the sidebar while the byte still reached the shell, and Ctrl+W closed the terminal tab instead of deleting a word. The registry also takes user overrides and reports clashing bindings, ready for a Settings editor. See `docs/feature-parity/shortcut-registry.json`.
- **The phones fold a turn's changed files behind "Changed N files".** Native Android and the React Native app used to show one row per changed file with no grouping. Each turn's files now collapse into one row with the turn's line totals, closed by default like the desktop, and a tap opens that turn's file rows on this device. See `docs/feature-parity/file-diff-cards-opt-in.json`.
- **Native Android's conversation list leads with the chats that need you.** The list is grouped into Needs you (an open approval, question or plan, or an error), Working and Done recently, with a plain grey count; the row dot carries the colour and the second line says what the chat waits on. Open cards come from the backend's record (`provider:get-pending-requests`, newest 25 chats) plus the live events, so a chat whose card opened before the app was listening still shows. A list with nothing waiting or working stays one plain list. See `docs/feature-parity/sidebar-needs-you.json`.
- **Native Android's conversation list previews the current turn.** Each row's second line shows the agent's `<agent_digest>` status, else a plain-text preview of its newest reply, by the same rule as the desktop sidebar and the phone (`TurnPreview.kt` ports `turnPreviewLine` and runs the shared test cases). The text comes from live events since the app started. See `docs/feature-parity/agent-digest.json`.
- **Native Android steers or queues a follow-up, and shows held messages as Queued.** A message sent while the agent works now goes out as the `provider:submit-user-turn` envelope (the positional `provider:send-turn` stays for a backend without `atomic_user_turn_v1`) and asks for `delivery: 'queue'` when the user picked it or the provider cannot steer, only on a backend with `turn_queue_v1`. The choice is frozen in the durable outbox, which gains a nullable `delivery` column (Room 5 to 6), so a retry sends the same fingerprint. Settings > Chat holds the device's "Follow-up while the agent works: Steer / Queue" default, and a chip above the composer flips it for one send. A held message's row reads Queued with Send now and Cancel (Cancel puts its text back in the composer) behind `turn_queue_controls_v1`, and the rows are re-listed after a reload or resume gap; on OpenCode, Send now is disabled and says why. See `docs/feature-parity/composer-follow-ups.json`.
- **Queued messages stay visible and can be sent now or taken back.** A message queued while the agent works used to look like any sent message and could not be changed. It now shows as a dashed bubble with a Queued chip and two actions: Send now steers it into the running turn, and Cancel removes it from the queue (and from the stored chat) and puts its text back in the composer. OpenCode cannot take a mid-turn message, so there Send now is disabled and says why. The backend announces held messages (`turn.queued` / `turn.dequeued`) and serves `provider:list-queued-turns`, `provider:promote-queued-turn` and `provider:cancel-queued-turn` behind the `turn_queue_controls_v1` capability, so every client stays in step; desktop and the React Native app re-list them after a reload or resume gap (native Android's queued rows are staged). Claude withdraws its queued message with the SDK's `cancelAsyncMessage` (verified live against Haiku), Codex promotes with `turn/steer`. The phone shows the same row with the same actions; native Android is staged behind `android_composer_follow_ups` and meanwhile decodes the events so they never show as unsupported. See `docs/feature-parity/composer-follow-ups.json`.
- **"Follow-up while the agent works: Steer / Queue".** A new setting in Settings > General (desktop) picks what Enter does with a message sent mid-turn; Steer stays the default, and Option+Enter does the other one. The phone has the same choice per device, under Connections, and its composer chip flips it for one send.
- **"Not in this chat" on the Follow chip.** The "Agent is working in <branch> Follow" chip can be muted per conversation, saved with the chat. A chat that has worked in more than two worktrees turns the chip off on its own and says so in one line, with Turn back on.
- **New worktrees clone `node_modules` instead of starting empty.** A fresh worktree used to have no dependencies, so the agent ran `npm install` and the machine ended up with one full copy per worktree - on the user's Mac, 6 worktrees under one repo held about 700MB of `node_modules` each, with the disk 94% full. Every worktree-creation flow now fires a background copy-on-write clone of the project's root `node_modules` right after the worktree exists: `cp -c -R` (APFS clonefile) on macOS, `cp -R --reflink=always` on Linux (btrfs/XFS only, since that is the only place a reflink can succeed). Before touching `cp` at all, it checks that the source and the worktree are on the same device and, on macOS, that the destination is actually on an APFS volume - `cp -c` does not fail when clonefile isn't available, it silently falls back to a real full copy (verified: cloning across an HFS+ volume with `cp -c -R` exits 0 and copies for real), so that check has to happen before the command runs, not be inferred from whether it succeeded. Any unsupported platform, cross-device pair, non-APFS macOS target, or command failure does nothing and logs once - there is no full-copy fallback, on purpose, since that would make the disk problem worse instead of better. The clone lands in a same-worktree staging directory and is atomically renamed into place only if `node_modules` is still absent, so a concurrent `npm install` in the same worktree can never be raced or clobbered by a failed or losing clone. Opt out per machine with the `worktree.cloneDependencies` setting. Measured on the user's Mac: cloning a live 934MB `node_modules` took 6.3-9.4s across runs and left `df`'s used space at Gi resolution unchanged before and after. See `docs/feature-parity/worktree-dependency-clone.json`.
- **Agent digest status previews.** The sidebar session list and kanban cards used to preview a chat with the first, often mid-sentence, words of the raw assistant reply. Claude and Codex now get a prompt rule asking them to report a short status line - "Writing cost-per-model table, 2 of 4 providers done", "Done: 6 review fixes pushed, tests green" - wrapped in `<agent_digest>` tags; the sidebar Recents row and the kanban card tile show that line instead, falling back to today's raw truncated preview when the agent reports none. The tag itself is stripped from the chat transcript on every surface (desktop, mobile, native Android) so it never shows raw, on the phone or anywhere else. OpenCode's ACP has no instructions seam to carry the rule, so it keeps today's raw preview; native Android's own list preview is staged behind `agent_digest_android_list_preview` pending a live per-thread text accumulator, though its chat-text stripping ships unconditionally. See `docs/feature-parity/agent-digest.json`.
...
- **Steer or queue a message while the agent works.** A steered message is read at the agent's next step (Claude and Codex). A queued one posts at once, the backend holds it, and it runs as its own turn when the current one ends. Enter does the configured follow-up default (Steer unless changed) and Alt+Enter the other one. It survives switching or closing the pane and keeps its images and pills. OpenCode cannot take a mid-turn message, so there it always queues. On the phone, a chip above the composer flips the next send to the other delivery.

### Changed
- The composer no longer shows "Enter send · Shift+Enter newline" under the input. The Send button's tooltip names the keys.
- **Accounts & models shows every account's room left at a glance.** The Providers list becomes one card per account, grouped by agent and sorted by the room left, under a strip naming the account with the most room, the next reset and how many accounts are signed out or failing. Each card shows its usage windows as wide bars (amber from 75%, red from 90%) with a reset countdown, a star on the default account, and one muted line for the model and credential folder; usage is read for every account when the page opens instead of behind a Usage button. Every action moved into a ⋯ menu: set as default, default model, rename or edit, test the sign-in, refresh usage, copy the folder path and delete (confirmed in-app). A signed-out account shows the reason and one Sign in again button, which copies its login command.
- **Icon composer.** The desktop composer's Stop / Queue / Steer / Send text buttons are now two small round icon buttons: Stop (a filled square) while a turn runs, and one Send arrow that does your follow-up default. Its tooltip names both keys, for example "Steer (Enter) · Queue (⌥Enter)", so the long hint shown under the composer while a turn runs is gone.
- **Plain Recents counts.** The Needs you and Working group counts in the sidebar are grey text instead of coloured pills; the row dot keeps the colour.
- **Menus, popovers and dialogs run on Radix.** The provider picker and branch picker are Radix popovers; the command palette, session picker, search, quick prompt, sidebar text prompt, kanban card and worktree dialogs, sidebar organizer and Settings are Radix dialogs. Radix now does the positioning, outside click, Escape, focus trap and stacking under the in-app confirm that each of them did by hand, and a dialog hands focus back to what had it on open (or to the composer). Every screen looks the same except the command palette, which fits its list instead of leaving a blank strip at the bottom, and the kanban card and worktree dialogs, which painted a theme variable that does not exist and let the board show through: they are opaque now. Escape in the provider picker's custom model field and in Settings' launch-config name fields backs out of the field instead of closing everything, and clicking the branch chip while its list is open closes it instead of reopening it. The slash-command and @-mention menus stay as they are, since the editor drives them and has focus the whole time. See `docs/feature-parity/radix-overlays.json`.
- **In-app confirm dialogs and shared button primitives.** Every `window.confirm` in the desktop renderer (16 call sites: removing a project, machine, workspace, provider instance or launch config, deleting kanban worktrees, the composer's delivery-recovery and draft-replace prompts, the profile-switch and fork-from-dirty-HEAD prompts, and the launch-config switch that kills busy panes) now opens one themed, promise-based dialog instead of a native macOS alert that froze the renderer and that Playwright could not click. The wording of each prompt is unchanged; destructive ones get a red action button. The composer's Stop and Send buttons share one height through a new `Button` primitive (shadcn/ui on Tailwind 4 utilities, scoped to `src/renderer/components/ui`, no preflight), so no other screen moved. The visual suite gains a confirm-dialog screen in every theme. See `docs/feature-parity/ui-primitives-confirm.json`.
- **The sidebar leads with the chats that need you.** Recents rows are two lines now: a status dot, the title and the time, then the agent's status line in full instead of squeezed beside the title. The list is grouped into Needs you (an open approval, question or plan, or an error), Working and Done recently; Needs you and Working always show in full with a count, even for an old chat, and Show N more pages through the rest as before. Needs you is read from the backend's record of open cards (`provider:get-pending-requests` plus the live events), so a chat you never opened this run still says it is waiting. The This Mac tree folds behind one row ("This Mac  2 workspaces, 12 projects"), offline remote machines fold behind "Remote machines  N offline", and both remember whether you opened them. Workspace counts read "14 projects", with the thread count on hover. The phone list already has the two-line row; its Needs you group is staged behind `mobile_conversations_needs_you`, and Android's behind `android_conversations_needs_you`. See `docs/feature-parity/sidebar-needs-you.json`.
- **In-chat file diff cards are opt-in, off by default.** The per-file diff cards after a turn went unread by most users, who only review a diff once it is a PR; a "Show file diff cards in chat" toggle in Settings > General (`chat.showFileDiffs`) now gates them. Off collapses a turn's changed files to a "Changed N files" button that expands the real cards, with per-hunk accept/reject, for that turn only. The backend still computes and emits `file.edited` for every turn regardless of this setting. Native Android and React Native/iOS already show a compact, ungrouped one-line-per-file summary with no per-hunk controls, so this ships desktop-only for now; grouping and collapsing that feed is staged behind `mobile_file_diff_collapse`. See `docs/feature-parity/file-diff-cards-opt-in.json`.
...
- **A tool call reads the same label on every client.** Desktop, mobile, and native Android each had their own tool-summary logic and disagreed: a shell call was "Bash" on desktop and Android but "Terminal" on mobile, a search was "Grep" or "Search" depending which client rendered it, and desktop's own path-shortener only collapsed macOS `/Users/...` paths while mobile's worked for any path. The rules now live in one place, `src/shared/tool-summary.ts` (moved from `apps/mobile/src/lib/toolSummary.ts`, same public API), and desktop's `ToolCallBlock.tsx` uses it for its collapsed-row label and detail; icons, colors and the expanded body stay desktop-only. Native Android has no TypeScript runtime, so `ThreadPresentation.kt`'s summarizer was hand-edited to match the same rules instead, and both are checked against one fixture file (`tests/fixtures/tool-summary-cases.json`) so they cannot silently drift apart again. Notable choices: mobile's generic, provider-neutral labels won over desktop's Claude-specific ones ("Terminal" not "Bash", "Search" not "Grep", "Find files" not "Glob", "Plan" not "Todos"); desktop's rename detection (an Edit with a `move_path` now shows "Rename" with an arrow) was folded in since neither mobile nor Android had it; Codex's real `update_plan`/`shell` tool names and OpenCode's ACP `search` tool-call kind are now recognized aliases. Native Android keeps three small, intentional divergences: MCP tool names stay humanized ("Notion · Search" instead of the raw `mcp__notion__search`), unknown tool names stay Title Cased, and every tool detail is capped at 140 characters where the shared rule caps a Task or Subagent detail at 80. All three were pre-existing and tested, so they were left alone rather than downgraded.

### Fixed
- **Every chat list shows each chat's summary line, not just the open one.** The desktop sidebar's Recents rows, kanban tiles, the phone list and the Android list only had a chat's status line while its messages were loaded, so most sidebar rows showed the project name. The backend now stores the last finished turn's line (agent digest, else the plain-text preview) in a new nullable `conversations.status_line` column when a turn ends, and serves it on the project and conversation lists. Existing chats get one the first time their history is opened. A live preview still wins while a chat is running or loaded. See `docs/feature-parity/persisted-status-line.json`.
- **Background-task notices survive a reload.** Claude queues some task notices and then drops them without ever writing the transcript's `<task-notification>` line, so a notice shown live disappeared on reload or switch-back. The backend now stores each live `task.notification` in the SQLite message mirror under the root conversation, one row per notice (a Monitor task reports once per event and again when it ends), with the event's status, summary, task id, output file and time. A reload pairs it with the transcript line for the same occurrence (equal task id, status and summary; a line without a task id must also be within 5s) and shows one row; with no line, the stored notice renders as the same compact row on desktop, the phone and Android. See `docs/feature-parity/persisted-task-notices.json`.
- **A closed "Follow suggestions are off" notice stays closed.** In a chat that had worked in more than two worktrees, the x on the notice only cleared it until the next drift check, which brought it straight back. The x is now saved with the conversation (new `follow_notice_dismissed` column, resolved through `resolveRootThreadId`), so the notice never returns in that chat however many worktrees it works in. The branch picker ends with "Turn Follow suggestions back on" while suggestions are off, so muting or closing stays reversible. The chip's own x still closes only the current suggestion, as before. See `docs/feature-parity/follow-notice-dismiss.json`.
- **Background-task notices show as they happen.** A Claude background task that finished, failed or was stopped only showed its compact "Background task failed: ..." row after the chat was reloaded, because the adapter ignored the SDK's live `system/task_notification`. It now forwards it as a `task.notification` event, and desktop, the React Native app and native Android render the same row the transcript produces on reload (status, summary, task id, output file). A reload shows it once. Codex has no live equivalent. See `docs/feature-parity/live-task-notifications.json`.
- **A Claude account idle for about 8 hours no longer shows as logged out in Settings > Providers.** Its access token had expired, but a refresh token was present, so chats kept working and `claude auth status` still said `loggedIn: true`, while the Usage panel reported it expired or rejected (HTTP 401) and offered the login command. One shared rule now decides "logged out": only a missing credential, a missing refresh token, or the CLI's own `loggedIn: false`. Usage asks the Claude CLI to refresh a stale token with `claude mcp list`, which makes no model call, then reads again; if that does not refresh it, the panel says "Signed in" and offers Refresh now, which sends one one-word message on the smallest model (a small amount of quota). Switchboard still never refreshes a token itself. Codex was not affected: its usage runs through `codex app-server`, which refreshes its own token. See `docs/feature-parity/expired-token-status.json`.
- **The composer's Stop and Send buttons sit inside the input box.** They were beside the box and straddled its bottom border, and the context ring under them sat a few pixels off. They now sit in the box's bottom-right corner with the same inset from its right and bottom edges, centred on a one-line draft and pinned to the corner as the draft grows; text and the placeholder stop short of them, and a tall draft's scrollbar ends above them. The context ring is the footer's last item, centred under Send, and stays on the right when a narrow pane wraps the footer. See `docs/feature-parity/composer-button-alignment.json`.
- **Picking a chat in the `/send-to` picker shows its name, keeps the caret, and the error banner can be closed.** A pick wrote the chat's raw id (`/send-to #7874da8c-...: `) into the composer. It now writes the title whenever that title resolves back to exactly the picked chat (unique among the chats you can message, no `:`), and falls back to `#<id>` when the title is ambiguous (another chat shares it) or cannot be written in the command (it contains `:`). The send still goes to the picked chat by id while the command names the picked title, so an auto-title rename between pick and send cannot redirect it. The picker also no longer offers chats that have not started, which the send could never reach. After a failed send, the composer's text used to come back with the caret at the start, so the next thing typed landed before `/send-to` (the old pick-and-send-on-one-Enter bug hit this every time); restored text now puts the caret at its end. The composer error banner clears once you edit the failed text or the next send succeeds, has a Dismiss (x) button, and "Nothing to send" offers "Write the message", which puts the caret after the colon. A failure that still has a Restore action stays until you restore or dismiss it. Desktop only: neither mobile client has `/send-to`. See `docs/feature-parity/send-to-pick.json`.
- **Numbered lists past 9 no longer lose the first digit, and a narrow chat no longer scrolls sideways.** An assistant list's markers sit in the list's 20px left padding, and the message clips its overflow, so "10." to "27." rendered as "0." to "7." with the first digit cut off. Ordered lists now get 2.5em, which fits three digits. In a narrow chat pane the Forward / Save / Copy row under each message could not wrap and widened the whole message list, which then scrolled sideways. It wraps now. A file pill in a sent message had a fixed 260px limit and widened the list the same way. It now truncates to the width of its line. Inline code and plain text already wrapped. Code blocks still scroll. The phone and Android renderers are not affected. See `docs/feature-parity/message-overflow.json`.
- **Reopening a chat keeps its tool rows and "Changed N files".** Switching away from a finished chat and back, or restarting the app, rebuilt the turn from the provider's transcript plus the SQLite mirror, and neither kept everything the live turn showed. The renderer built tool rows and changed-file cards live and never saved them, the mirror held only assistant text, and the Codex transcript parser reads messages only. So a Codex, OpenCode or demo chat came back with no "Used N tools" row, and every chat, Claude included, lost its "Changed N files" group. The backend now mirrors each tool call (with its output) and each changed-file card at turn end, the reload merge folds a mirrored tool call into Claude's transcript copy of it (which fills in the output the transcript lacks), accepting or rejecting a card is stored so it does not ask again, and a reverted card now checks the file still holds what the agent wrote, so a card reopened after later edits cannot undo them. Phone and Android rebuild their changed-file rows from the same history. Turns from before this fix stay as they were: Claude chats keep their transcript tool rows, and nothing else comes back. See `docs/feature-parity/reopen-tool-rows.json`.
- **A chat keeps its order after you switch away and back.** Coming back to a finished Claude or Codex chat could show some of the agent's work-in-progress text again below its final answer. The backend mirrored every assistant message of a turn to SQLite stamped with the turn end, and the reload merge only recognised a mirror row as the transcript's copy within 60s, so interim text from a long turn came back a second time and sorted last. Mirror rows now carry the time their last chunk arrived, which is when the transcript writes the same message, and rows already stored are matched to their transcript copy anywhere after the turn's last user message, so existing chats are fixed on the next open (except interim text written before a mid-turn steer in an old chat). Phone and Android get the fix from the same backend. See `docs/feature-parity/chat-render-order.json`.
- **Provider-generated user messages no longer render as "You" bubbles of raw XML.** Claude Code writes background-task notifications (`<task-notification>`), interrupt markers and local-command output into the transcript with the user role, and Codex does the same with its image wrappers and bootstrap context. A shared parser, `src/shared/synthetic-message.ts` (Kotlin port `SyntheticUserMessage.kt`), peels those blocks off the front of a user message: a task notification becomes a compact, colour-coded row such as "Background task failed: Build (exit 144)" whose task id and output file show on expand, an interrupt becomes "Interrupted", and pure context (caveats, reminders, environment, skill bodies) is dropped. Anything the user typed after the blocks keeps its bubble. Claude entries flagged `isMeta` (image size notes, skill bodies, "Continue from where you left off.") are dropped on reload, and neither kind can become a session title or a sidebar preview turn boundary. Desktop, mobile and native Android all render the rows. See `docs/feature-parity/synthetic-user-messages.json`.
- **Session previews are plain text.** The sidebar, kanban cards and phone list no longer show raw markdown such as backticks and asterisks.
...
### Added
- **The Archived tab has a search box and a pager**, 10 rows per page. It used to render every archived conversation in one unbounded list, so a long archive was a wall to scroll. Search matches the title and the project path, which are the two things the row shows; whitespace splits the query into terms that must all match, so "auth acme" finds an auth chat in acme-console, and a single term stays a plain substring test. The pager sits ABOVE the rows on purpose: a full page is taller than the modal, so underneath them it lands below the fold. Filtering and paging are pure in `settings/archived-list.ts`, because the interesting part is the clamping - the page count shrinks under the caller both when typing narrows the list and when unarchiving the last row of the last page removes that page, and either would otherwise render a valid page number as an empty list.
...
- **Resize dividers could get stuck in "resize mode."** If the pointer crossed into the code-server `<webview>` (the embedded IDE) or the xterm canvas mid-drag, pointer capture was lost, the terminating `pointerup` never arrived, and the `col-resize` cursor + `user-select: none` + `pointerEvents` overrides stayed frozen until reload. Fixes, applied to all three handles (main `ResizeHandle`, terminal `PaneResizeHandle`, dual-chat `ChatSplitHandle`):
  - A full-viewport shield overlay (`src/renderer/services/drag-overlay.ts`) is raised for the duration of a drag, so the pointer can't reach a child frame and capture is never lost in the first place.
...
- IPC: `app:fork-conversation` handler in `src/main/ipc/app.ts`, orchestration in `src/main/conversations/fork.ts`, renderer service in `src/renderer/services/fork-session.ts`, popover UI in `MessageBubble.tsx`. Concurrency guard: refuses to fork while the source session has a turn in flight.
