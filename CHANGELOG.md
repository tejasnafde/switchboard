...
### Added
- **Settings' dropdowns are app controls, and the project picker is searchable.** The native selects in Settings (the launch-config project, the two worktree setup policies, the mobile pairing host, Recent conversations and New chat environment) are now one combobox primitive, `components/ui/combobox.tsx` (Radix popover + cmdk). The project picker has a search box and lists recent projects first, then projects under their sidebar workspace; the short lists drop the search box. Arrows, Enter and Escape work, and focus returns to the control. Values and settings are unchanged. See `docs/feature-parity/settings-combobox.json`.
