...
### Added
- **Shortcuts can be changed in Settings > Keyboard.** Click a shortcut and press the new keys; Escape cancels and Backspace leaves the command without a key. A changed row shows "Changed" with a Reset, and Reset all puts every shortcut back. A chord the OS owns (⌘Q, ⌘Space, the screenshot keys), that text editing or the terminal needs (⌘C, Ctrl+D, a bare letter or Enter, any Ctrl key on macOS) or that another command already uses is refused with the reason. The composer's Enter / Shift+Enter / ⌥Enter, the find bar keys and the numbered ranges stay fixed. Rebinds are stored as one settings value (`keyboard.overrides`) and take effect on the next key press, including the app menu, which is rebuilt; a filter box finds a shortcut by name or key. See `docs/feature-parity/shortcut-rebinding.json`.
